...
def encode_feedparser_dict(fp_dict):
    """
    ...
    if isinstance(fp_dict, feedparser.FeedParserDict) or isinstance(fp_dict, dict):
        ...
        for key in fp_dict.keys():
            ret_dict[key] = encode_feedparser_dict(fp_dict[key])
        return ret_dict
    elif isinstance(fp_dict, list):
        dict_list = []
        for key in fp_dict:
            dict_list.append(encode_feedparser_dict(key))
        return dict_list
    else:
        return fp_dict
    
def extract_relevant_info(fp_dict):
    ret_dict = {}
...
def run_query(search_query, start=0, max_results=10):
...
def fetch_papers(
    ...
    for i in range(start_index, max_index, results_per_iteration):
        ...
        parsed_response = run_query(search_query, i, results_per_iteration)
        ...
        for entry in parsed_response.entries:
            ...
            dict_entry = extract_relevant_info(entry)

            rawid, version = parse_arxiv_url(dict_entry['link'])
